...
#pragma warning restore CS0618
        ...
        public async Task AppExitAsync(bool needShutdown)
        {
            ...
            {
                // Check if running on macOS and terminate any remaining xray processes
                if (Utils.IsOSX() && needShutdown)
                {
                    await KillXrayProcessAsync();
                }
        ...
        /// <summary>
        /// Kill any remaining xray processes on macOS
        /// </summary>
        private async Task KillXrayProcessAsync()
        {
            try
            {
                Logging.SaveLog("Attempting to kill any remaining xray processes on macOS...");
                
                // Start the process to execute the pkill command
                var startInfo = new System.Diagnostics.ProcessStartInfo
                {
                    FileName = "/bin/bash",
                    Arguments = "-c \"pkill -f xray\"",
                ...
                using (var process = System.Diagnostics.Process.Start(startInfo))
                {
                    if (process != null)
                    {
                        ...
                        {
                            Logging.SaveLog($"pkill xray error: {error}");
                        }
                        else if (!string.IsNullOrEmpty(output))
                        {
                            Logging.SaveLog($"pkill xray output: {output}");
                        }
                        else
                        {
                            Logging.SaveLog("Successfully killed xray processes or no processes to kill.");
                        }
                    }
                }
            }
            ...
            {
                Logging.SaveLog($"Failed to kill xray process: {ex.Message}");
            }
        }
        ...
        {
            var sql = @$"select a.IndexId ,a.ConfigType ,a.Remarks ,a.Address ,a.Port ,a.Network ,a.StreamSecurity ,a.Subid ,b.remarks as subRemarks from ProfileItem a left join SubItem b on a.subid = b.id where 1=1 ";
            if (subid.IsNotEmpty())
            {
                ...
                try
                {
                    ...
                    switch (item.ConfigType)
                    {
                        ...
                        case EConfigType.TUIC:
                            extra = extra with
                            {
                                CongestionControl = item.HeaderType.NullIfEmpty(),
                            };
