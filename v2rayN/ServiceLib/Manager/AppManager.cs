...
public sealed class AppManager
{
    ...
    {
        get { _statePort ??= Utils.GetFreePort(GetLocalPort(EInboundProtocol.api)); return _statePort.Value; }
    }

    public int StatePort2
    {
        get { _statePort2 ??= Utils.GetFreePort(GetLocalPort(EInboundProtocol.api2)); return _statePort2.Value + (_config.TunModeItem.EnableTun ? 1 : 0); }
    }
    ...
    public async Task AppExitAsync(bool needShutdown)
    {
        try
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                ...
                {
                    Logging.SaveLog($"pkill -f xray failed: {ex.Message}");
                }
            }
...
#pragma warning restore CS0618
    ...
    public List<string> GetShadowsocksSecurities(ProfileItem profileItem)
    {
        ...
        switch (coreType)
        {
            case ECoreType.v2fly: return Global.SsSecurities;
            case ECoreType.Xray: return Global.SsSecuritiesInXray;
            case ECoreType.sing_box: return Global.SsSecuritiesInSingbox;
        }
